...
/********************************************************************************************************/
	...
	case ASM_COMMAND_STOP:
		...
		if (handle == asm_rcv_msg.data.cmd_handle) {
			...
			if (asm_rcv_msg.data.reason_of_blocking == ASM_SOUND_BLOCKED_BY_TABLE) {
				/* check previous sound event */
				switch (asm_rcv_msg.data.previous_sound_event) {
				case ASM_EVENT_ALARM:
				...
				case ASM_EVENT_VOIP:
				case ASM_EVENT_RICH_CALL:
					debug_warning("blocked by CALL/VIDEOCALL/RICH_CALL");
					*error_code = ERR_ASM_POLICY_CANNOT_PLAY_BY_CALL;
					break;
				default:
					debug_warning("blocked by Other(sound_event num:%d)", asm_rcv_msg.data.previous_sound_event);
					*error_code = ERR_ASM_POLICY_CANNOT_PLAY;
					break;
				}
			} else if (asm_rcv_msg.data.reason_of_blocking == ASM_SOUND_BLOCKED_BY_SOUND_PROFILE) {
				debug_warning("blocked by sound profile, previous sound_event:%d", asm_rcv_msg.data.previous_sound_event);
				*error_code = ERR_ASM_POLICY_CANNOT_PLAY_BY_PROFILE;
			} else if (asm_rcv_msg.data.reason_of_blocking == ASM_SOUND_BLOCKED_BY_CUSTOM) {
				debug_warning("blocked by custom reason, previous sound_event:%d", asm_rcv_msg.data.previous_sound_event);
				*error_code = ERR_ASM_POLICY_CANNOT_PLAY_BY_CUSTOM;
			} else {
				debug_warning("could not find reason, previous sound_event:%d", asm_rcv_msg.data.previous_sound_event);
				*error_code = ERR_ASM_POLICY_CANNOT_PLAY;
			}
			return false;
		} else {
                            ...
                            ASM_sound_states_t interest_sound_state, ASM_watch_cb_t callback, void *user_data, int *error_code)
{
	...
	*error_code = ERR_ASM_ERROR_NONE;
	...
	if (interest_sound_event < ASM_EVENT_SHARE_MMPLAYER || interest_sound_event >= ASM_EVENT_MAX) {
		*error_code = ERR_ASM_EVENT_IS_INVALID;
